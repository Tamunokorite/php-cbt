...
$db = new Database;
$db->query("SELECT * FROM questions WHERE subject=:id");
$db->bind(':id', $_GET['id']);
$data = $db->resultSet();
if($db->rowCount() <= 0) {
    $data = [];
...
<div class="wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <h1 class="m-0">All Questions (<?php echo count($data); ?>)</h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
            ...
            <?php foreach ($data as $key => $val) :?>
                <div class="card">
                    <div class="card-header">
                        <h5 class="m-0"><?php echo $key+1 . ". " . $val->content ;?></h5>
                    </div>
                    <div class="card-body">
                        <?php 
                            $db->query("SELECT * FROM choices WHERE question=:question");
                            $db->bind(':question', $val->id);
                            $choices = $db->resultSet();
                            $letters = ['A', 'B', 'C', 'D'];
                        ?>
                        
                        <?php foreach ($choices as $choice_key => $choice) :?>
                            <p class="<?php $class = $choice->isCorrect == 1 ? 'text-success' : 'text-dark'; echo $class; ?>"><?php echo $letters[$choice_key] . ". " .$choice->content; ?></p>
                        <?php endforeach; ?>
                    </div>
                    <div class="card-footer">
                        <a href="editquestion.php?id=<?php echo $val->id; ?>" class="btn btn-dark">Edit</a>
                        <a href="deletequestion.php?id=<?php echo $val->id; ?>&subject_id=<?php echo $_GET['id']?>" class="btn btn-dark float-right">Delete</a>
                    </div>
                </div>
            <?php endforeach; ?>
          </div>
  ...
  <footer class="main-footer">
    ...
    <div class="float-right d-none d-sm-inline">
      <a href="subjects.php">Go back</a><a href="logout.php">Logout</a>
    </div>
    <!-- Default to the left -->
